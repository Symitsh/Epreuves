...
class Labyrinth
  ...
  # Retourne les coordonnées de la porte '1' dans le labyrinthe
  def find_gate_one
  ...
  # retourne les coordonnées de la porte 2 dans le labyrinthe
  def find_gate_two
    @labyrinth.each_with_index do |line, i|
      line.each_with_index do |char, j|
        next unless char == '2'
  ...
  # Utilise une recherche en largeur (BFS) pour trouver le chemin le plus court entre les deux portes dans le labyrinthe
  def solve_labyrinth
  ...
  # Vérifie les cases voisines de la case donnée et ajoute les cases valides à la file d'attente
  ...
  # Retourne les coordonnées des cases voisines de la case donnée, en vérifiant que ces coordonnées sont valides
  ...
  # Retourne l'élément de la case donnée dans le labyrinthe.
  def get_element(coordinates)
    @labyrinth[coordinates[0]][coordinates[1]]
  end

  # Trace le chemin trouvé dans le labyrinthe original et le retourne.
  def solution(path)
    result = @labyrinth
    @result_final = 0
    path.each do |coords|
  ...
  # Retourne le chemin entre les deux portes en suivant les "précédents" enregistrés lors de la recherche en largeur.
  def trace_path
...
def main
  ...
  puts lines[0].join
  my_labyrinth = Labyrinth.new(lines[1..])
